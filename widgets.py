...
@Description: 
@Author: Allen
@Date: 2020-04-03 19:07:53
@LastEditTime: 2020-04-06 00:40:58
@LastEditors: Allen
'''
...
# entry and text
lf_entry_text = tk.LabelFrame(root,
                              width=160,
                              height=240,
                              text='Entry and Text')
...
button2 = tk.Button(lf_entry_text, text='inset curr', command=button2_click)
button2.place(x=6, y=32)

button3 = tk.Button(lf_entry_text, text='inset end', command=button3_click)
button3.place(x=80, y=32)

textbox = tk.Text(lf_entry_text, width=20, height=10, relief='solid')
...
listbox = tk.Listbox(lf_Listbox, listvariable=myList, height=11)

# 为Listbox关联scrollbar
scrollbar = tk.Scrollbar(lf_Listbox,
                         width=30,
                         orient="vertical",
...
menubutton = tk.Menubutton(lf_MenuButton, text="Fruits", relief='raised')
...
optionmenu = tk.OptionMenu(lf_OptionMenu, op_var, *Options)
...
spinbox = tk.Spinbox(lf_SpinBox, from_=0, to=10, width=8)
